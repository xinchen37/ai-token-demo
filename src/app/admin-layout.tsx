...
const navGroups = [
  ...
  label: string;
  items: Array<{ key: PageKey; label: string; icon: React.ComponentType<{ className?: string }> }>;
}>;
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-[300px] border-r border-slate-100 bg-white lg:block">
        ...
        <div className="mx-4 flex h-16 items-center gap-3 rounded-md border border-slate-200 bg-white px-3 shadow-sm shadow-slate-100">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-medium text-slate-800">Puling</div>
            ...
            <div key={`${group.label}-${groupIndex}`} className="border-t border-dashed border-slate-100 first:border-t-0">
              ...
              <div className="space-y-1">
                {group.items.map((item, itemIndex) => {
                  const selected = activePage === item.key && (item.key === "dashboard" || itemIndex === 0);
                  return (
                    <button
                    ...
                    >
                      ...
                      {item.key !== "dashboard" ? (
                        <ChevronDown className={cn("size-4", selected ? "text-white/80" : "text-slate-400")} />
                      ) : null}
                    </button>
                  );
                })}
          ...
          <header className="sticky top-4 z-20 flex h-16 items-center justify-between rounded-md border border-slate-200 bg-white px-4 shadow-sm shadow-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                {activePage === "dashboard" ? <Database className="hidden size-5 text-slate-400 sm:block" /> : null}
