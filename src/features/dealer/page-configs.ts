...
export const pageConfigs = {
  models: {
    ...
    columns: [
      ...
      { key: "name", label: "模型名称" },
      { key: "type", label: "模型类型" },
      { key: "inputPrice", label: "输入价格", format: (value) => `${formatCurrency(Number(value))}/1M` },
      { key: "outputPrice", label: "输出价格", format: (value) => `${formatCurrency(Number(value))}/1M` },
    ...
    fields: [
      ...
      { key: "costInputPrice", label: "成本输入价", kind: "number" },
      { key: "costOutputPrice", label: "成本输出价", kind: "number" },
  ...
  products: {
    ...
    columns: [
      ...
      { key: "relatedModels", label: "关联模型" },
      { key: "tokenLimitM", label: "Tokens(M)" },
  ...
  contracts: {
    ...
    fields: [
      ...
      { key: "orderedAt", label: "下单时间", kind: "datetime" },
      { key: "dailyLimit", label: "每日限额" },
      { key: "expiresAt", label: "过期时间", kind: "datetime" },
